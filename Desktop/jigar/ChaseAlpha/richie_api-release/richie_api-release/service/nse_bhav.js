class BhavCopy {
    ...
    __generateFileNames(criteria) {
        ...
        const baseUrl = (this.marketType === "FO") ? "https://archives.nseindia.com/content/historical/DERIVATIVES/" : "https://archives.nseindia.com/content/historical/EQUITIES/" ;
        const filePrefix = (this.marketType === "FO") ? "fo" : "cm"
        ...
        if (!day || day === undefined) {
            for (let i = 1; i <= 31; i++) {
                ...
                fileName = filePrefix + day + month + year + "bhav" + ".csv.zip";
                url = baseUrl + year + "/" + month + "/" + fileName;
                allFilesPathInMonth.push(url);
            }
        } else {
            const fileName = filePrefix + day + month + year + "bhav" + ".csv.zip";
