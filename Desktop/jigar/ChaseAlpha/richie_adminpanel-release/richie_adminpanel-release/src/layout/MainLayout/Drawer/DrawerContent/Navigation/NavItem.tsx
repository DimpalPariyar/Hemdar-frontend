...
const NavItem = ({ item, level }: Props) => {
  ...
  const iconSelectedColor = theme.palette.mode === 'dark' && drawerOpen ? 'grey.400' : '#2C00D3';
  const backgroundCol = theme.palette.mode === 'dark' && drawerOpen ? 'grey.400' : '#E5E9FF';
  return (
    <ListItemButton
      {...listItemProps}
      ...
      sx={{
        ...
        ...(drawerOpen && {
          '&:hover': {
            bgcolor: theme.palette.mode === 'dark' ? 'divider' : backgroundCol,
            borderTopLeftRadius: '25px',
            borderBottomLeftRadius: '25px'
          },
          '&.Mui-selected': {
            bgcolor: theme.palette.mode === 'dark' ? 'divider' : backgroundCol,
            borderRight: `10px solid ${iconSelectedColor}`,
