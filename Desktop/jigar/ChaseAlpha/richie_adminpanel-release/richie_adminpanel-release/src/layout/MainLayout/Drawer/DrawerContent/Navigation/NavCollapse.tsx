...
const NavCollapse = ({ menu, level }: Props) => {
  ...
  const levelUser = UserLevel?.map((x: any) => x.value)
  const navCollapse = menu.children?.map((item) => {

    switch (item.type) {
      case 'collapse':
        return levelUser.includes(item.level) ? <NavCollapse key={item.id} menu={item} level={level + 1} /> : "";
      case 'item':
        return levelUser.includes(item.level) ? <NavItem key={item.id} item={item} level={level + 1} /> : "";
      default:
        return (
  ...
  const iconSelectedColor = theme.palette.mode === 'dark' && drawerOpen ? "white" : "#2C00D3";
    ...
    <>
      <ListItemButton
        ...
        sx={{
          ...
          ...(drawerOpen && {
            '&:hover': {
              bgcolor: theme.palette.mode === 'dark' ? 'divider' : '#E5E9FF',
      ...
      >
        {menuIcon && (
          <ListItemIcon
            style={{ color: selected === menu.id ? iconSelectedColor : textColor }}
            sx={{
              minWidth: 28,
        ...
        {(drawerOpen || (!drawerOpen && level !== 1)) && (
          <ListItemText
            primary={
              <Typography variant="h6" color={selected === menu.id ? iconSelectedColor : textColor}>
                {menu.title}
              </Typography>
            }
            secondary={
        ...
        {(drawerOpen || (!drawerOpen && level !== 1)) &&
          (openMini || open ? (
            <UpOutlined style={{ fontSize: '0.625rem', marginLeft: 1, color: iconSelectedColor }} />
          ) : (
