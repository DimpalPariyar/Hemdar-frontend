...
// assets
import { CloseSquareFilled, DragOutlined } from '@ant-design/icons';
...
export const HeaderSort = ({ column, sort }: HeaderSortProps) => {
  ...
  return (
    <Stack direction="row" spacing={1} justifyContent="space-between" alignItems="center" sx={{ display: 'flex' }}>
      <Box>{column.render('Header')}</Box>
      {column.Header === "Status" ? (<div ><button>Status</button></div>) : ""}
    </Stack>
  );
};
