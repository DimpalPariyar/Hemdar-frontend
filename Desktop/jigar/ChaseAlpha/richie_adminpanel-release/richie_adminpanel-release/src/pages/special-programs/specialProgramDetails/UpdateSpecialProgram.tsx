...
const UpdateProgram = ({ handleClose }: Props) => {
  const dispatch = useDispatch();
  const theme = useTheme();
  const { programId, currentProgramDetails } = useSelector((state: { specialProgram: any }) => state.specialProgram);

  const [currentRows, setCurrentRows] = useState<any>({
    programTitle: currentProgramDetails?.programTitle,
    shortDescription: currentProgramDetails?.shortDescription,
    bannerImage: currentProgramDetails?.bannerImage,
    faqTitle: '',
    faqs: {}
  });
  ...
  useEffect(() => {
    const current = currentRows;
    currentProgramDetails?.faqs?.map((faq: any) => {
      const id = UIDV4();
      const newRow = {
        faqQuestion: faq?.question,
        faqAnswer: faq?.answer
      };
      current.faqs[id] = { ...newRow };
    });
    setCurrentRows({ ...current });
  }, []);
  ...
  const handleSubmit = async (values: any) => {
    ...
    axios
      .put(`${BASE_URL}/learning/program/${programId}`, {
      ...
      .then(async () => {
        dispatch(
          openSnackbar({
            open: true,
            message: 'Special Program is updated successfully',
            variant: 'alert',
            alert: {
              color: 'success'
            },
      ...
      <Formik initialValues={currentRows} validationSchema={new FAQValidationSchema().faqSchema} onSubmit={handleSubmit}>
        {(formik) => {
          return (
            <Form>
              <Box sx={{ p: 1, py: 1.5 }}>
                <DialogTitle>Update the Special Program</DialogTitle>
                <DialogContent>
