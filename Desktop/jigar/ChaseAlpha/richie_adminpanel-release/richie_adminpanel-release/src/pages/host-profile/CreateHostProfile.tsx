...
const CreateHostProfile = ({ handleClose }: any) => {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/hostProfile/types`).then((response: any) => {
      ...
      for (let i = 0; i < values.length; i++) {
        let item: any = {};
        item['id'] = Number(keys[i]);
        item['displayName'] = values[i].label;
    ...
    axios
      ...
      .then(async () => {
        dispatch(
          openSnackbar({
            open: true,
            message: 'New Host is created successfully',
            variant: 'alert',
            alert: {
              color: 'success'
            },
          ...
          return (
            <Form>
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Stack spacing={1}>
                    <DialogTitle>Create New Host Profile</DialogTitle>
                    <DialogContent>
                      <Stack spacing={1}>
